...
- [[2026-08-21-release-datastore-rds-to-statefulset]] — 비용/관리 부담으로 release의 RDS/ElastiCache를 완전히 없애고 기존 `dev-datastore`(EBS StatefulSet MySQL/Redis)로 대체(prod는 그대로 RDS/ElastiCache 유지). MySQL root 비밀번호를 `02_k8s-addon`(매일 밤 재생성)에서 `03_registry`(영구)로 이전해 잠재 드리프트 버그 제거. `db-secrets`/`redis-secrets`는 처음엔 ESO 재사용으로 갔다가, RDS와 달리 로테이션이 필요 없다는 걸 근거로 `modules/addons/eso`에 `manage_db_redis_secrets` 스위치를 추가해 release는 plain `kubernetes_secret`으로 직접 관리하도록 최종 변경 — DB 비밀번호는 사실상 영구 고정(장기 키)으로 의도적으로 수용(클러스터 내부 전용 네트워크라 노출 경로 제한적). **CD 쪽 미착수**: `values.yaml`에 `DB_HOST`/`REDIS_HOST` 하드코딩 + `backend.secrets`에서 `redis-secrets` 제거 필요 — apply 시 release의 현재 RDS/ElastiCache 데이터가 삭제됨(사용자 명시 동의), 다음 아침 실제 plan/apply로 최종 검증 필요
- [[2026-08-21-04data-split-release-prod-directories]] — 위 변경으로 release/prod 구조 자체가 달라지면서(count/삼항식이 늘어남), `04_data`(단일 root+workspace)를 `04_data/release`·`04_data/prod` 완전 독립 두 root로 분리(사용자 판단, 완전 중복 방식 채택). release는 기존 workspace가 쓰던 S3 state key를 그대로 재사용해 상태 이전 없이 이어받음(`terraform state list`로 실증 확인), prod는 한 번도 apply된 적 없어 새 key 사용. **분리 중 이번 작업과 무관한 잠재 버그 발견**: `module.eso`의 ESO 컨트롤러(helm_release)가 release/prod 구분 없이 고정 name/namespace라, prod를 처음 apply하면 release가 이미 설치한 것과 충돌할 가능성 — ✅ 해결됨([[2026-08-21-eso-controller-shared-singleton]])
- [[2026-08-21-eso-controller-shared-singleton]] — 위에서 발견된 ESO 컨트롤러 충돌 버그(이름 충돌 `EntityAlreadyExists` + CRD 소유권 충돌 `invalid ownership metadata`, prod 첫 apply에서 실제 재현)를 해결. `modules/addons/eso-controller`(신규)로 컨트롤러(Helm 릴리즈+CRD+IRSA 역할)만 빼서 `02_k8s-addon`에 진짜 공유 singleton으로 설치, `modules/addons/eso`(release/prod 각자)는 그 공유 역할 이름만 받아 SecretStore/ExternalSecret + 자기 시크릿 읽기 정책만 담당하도록 재설계. 부수 효과로 [[crd-not-yet-installed-on-fresh-apply]]가 요구하던 "04_data의 module.eso를 먼저 apply" 런북 절차도 함께 해소
...
- [[ebs-csi-addon-destroyed-before-dev-datastore-pvc]] — "컨트롤러가 자기 CR/PV보다 먼저 destroy돼서 finalizer가 영원히 안 풀리는" 문제, 같은 날 두 번 재현: (1) `aws_eks_addon.ebs_csi`가 `module.dev_datastore`보다 먼저 지워져 dev-mysql/redis PVC/PV가 멈춤, (2) `module.keda`가 `kubernetes_namespace.qket`보다 먼저 지워져 ArgoCD가 만든 ScaledObject 4개가 멈춰서 네임스페이스 자체가 안 지워짐 — 둘 다 finalizer 강제 제거로 즉시 해결(Retain 정책/네임스페이스 재생성 전제라 안전) + `depends_on` 추가(`module.eso_controller`도 선제적으로 포함)로 재발 방지, 일반화된 체크리스트까지 정리 — **해결됨**
- [[gatewayclass-alb-controller-destroy-order]] — 같은 클래스의 세 번째 사례: `GatewayClass`가 ALB Controller 자신의 finalizer를 갖고 있는데, CRD 차트에 같이 묶여있어서 "생성은 컨트롤러보다 먼저, 파괴는 컨트롤러보다 나중"이라는 반대 방향 요구가 동시에 걸려있었음 — `depends_on` 하나로 둘 다 못 만족시키는 구조적 문제라 GatewayClass를 별도 리소스로 분리(반대 방향 `depends_on`)해서 근본 해결. 덤으로 이 차트에 잘못 들어있던 untracked 파일(`gateway.yaml`, pilot 모듈 것이 실수로 복사됨)이 렌더링 자체를 깨뜨리고 있던 것도 같이 발견/삭제 — **해결됨**
...
- ✅ (해결+적용 완료, 2026-08-21) [[decisions/2026-08-21-04data-split-release-prod-directories]]에서 발견한 잠재 버그(`modules/addons/eso`의 ESO 컨트롤러가 release/prod 구분 없이 고정 name/namespace를 써서 충돌) — prod 첫 apply에서 실제 재현(`EntityAlreadyExists` → `invalid ownership metadata`)된 뒤 [[decisions/2026-08-21-eso-controller-shared-singleton]]으로 설계 확정 + 라이브 클러스터에 무중단 적용까지 완료(state rm+import로 release 기존 리소스 이전, prod 고아 역할 정리, 컨트롤러 파드 재시작 0회)
- `04_data/release`·`04_data/prod` 분리(같은 문서) 이후 실제 `terraform plan`/`apply`는 아직 한 번도 안 돌려봄 — `03_registry`(신규 `dev_mysql_root_secret_arn` 출력)도 아직 apply 전이라, 다음 세션에서 `03_registry` → `04_data/release` 순서로 apply하고 plan 결과(RDS/ElastiCache/SG가 정확히 destroy 목록에만 뜨는지)를 확인해야 함
- 🔴 (신규, 2026-08-21) [[decisions/2026-08-21-release-datastore-rds-to-statefulset]]로 release의 `db-secrets`/`redis-secrets` 관리 방식이 바뀌면서 **CD 레포(`values.yaml`) 쪽 대응이 아직 미착수** — `backend.secrets` 목록에서 `redis-secrets` 제거만 하면 됨(`DB_HOST`/`REDIS_HOST`는 이미 Terraform이 `app-config` ConfigMap에 넣어주고 CD가 그 ConfigMap을 이미 참조하고 있어서 CD 쪽 추가 하드코딩은 필요 없음). 안 하면 release 배포 시 존재하지 않는 `redis-secrets`를 참조해서 파드가 못 뜰 수 있음(사용자가 직접 처리하기로 함)
