...
- [[backend-cold-start-cpu-contention-during-rollout]] — 새 backend 파드 여러 개가 한 노드에 몰려서 뜰 때 JVM 콜드스타트 CPU 경합으로 liveness/readiness probe가 실패하는 문제, KEDA 스케일업(자가치유)과 설정변경발(發) 전체 롤링배포(`HealthyHostCount`가 1로 붕괴, 실제 로그인 실패로 이어짐) 두 갈래로 재현 — 근본 해결(topologySpreadConstraints 등)은 미착수
...
- ✅ (해결, 2026-08-21) [[decisions/2026-08-21-04data-split-release-prod-directories]]에서 발견한 잠재 버그(`modules/addons/eso`의 ESO 컨트롤러가 release/prod 구분 없이 고정 name/namespace를 써서 충돌) — prod 첫 apply에서 실제 재현(`EntityAlreadyExists` → `invalid ownership metadata`)된 뒤 [[decisions/2026-08-21-eso-controller-shared-singleton]]으로 해결. **아직 남은 작업**: release가 이미 만들어둔 environment-접미사 IAM 역할/Helm 릴리즈(`team5-qket-eso-role-release` 등)를 새 공유 설계로 전환하기 전에 수동 정리 필요 — 해당 문서의 "트레이드오프/남은 리스크" 참고
