...
- ✅ (해결, 2026-08-19) backend actuator 헬스체크 스레드풀 공유 문제([qKet/backend#29](https://github.com/qKet/backend/issues/29)) — `application.yml`에 `management.server.port: 8081` 추가로 분리, `Infra/02_k8s-addon/main.tf`(ALB Ingress `healthcheck-port` 애노테이션), `CD/helm/templates/backend-deployment.yaml`(Service·containerPort 8081 추가), `CD/helm/templates/networkpolicy.yaml`+`values.yaml`(8081 허용)까지 4개 레포 동반 수정 완료 — **코드 반영됨, 배포(CI 빌드+ArgoCD sync)는 아직**
- 🔴 (신규, 2026-08-18, 최우선) frontend 헬스체크 경로가 무거운 SSR 페이지(`/`)를 그대로 씀([qKet/frontend#28](https://github.com/qKet/frontend/issues/28)) — ALB 타겟그룹 헬스체크 연쇄 장애의 나머지 절반(backend는 위 항목으로 해결). [[troubleshooting/loadtest-10000-open-run-cascading-failures]] "문제 4" 참고 — **팀원이 별도 진행 중**
