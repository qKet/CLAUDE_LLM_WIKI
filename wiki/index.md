...
- 🔴 (신규, 2026-08-21) [[decisions/2026-08-21-release-datastore-rds-to-statefulset]]로 release의 `db-secrets`/`redis-secrets` 관리 방식이 바뀌면서 **CD 레포(`values.yaml`) 쪽 대응이 아직 미착수** — `DB_HOST=dev-mysql`/`REDIS_HOST=dev-redis` 하드코딩 추가, `backend.secrets` 목록에서 `redis-secrets` 제거 필요. 안 하면 release 배포 시 REDIS_HOST가 안 들어가거나 존재하지 않는 Secret을 참조해서 파드가 못 뜰 수 있음(사용자가 직접 처리하기로 함)
