...
- [[2026-08-21-release-datastore-rds-to-statefulset]] — 비용/관리 부담으로 release의 RDS/ElastiCache를 완전히 없애고 기존 `dev-datastore`(EBS StatefulSet MySQL/Redis)로 대체(prod는 그대로 RDS/ElastiCache 유지). `04_data`에 이 레포 최초의 환경별 조건부 모듈 생성(`count`) 패턴 도입, `module.eso`는 코드 변경 없이 입력값만 바꿔치기(RDS 마스터 시크릿과 같은 JSON 모양으로 맞춘 신규 `03_registry` 시크릿 재사용), MySQL root 비밀번호를 `02_k8s-addon`(매일 밤 재생성)에서 `03_registry`(영구)로 이전해 잠재 드리프트 버그 제거 — **apply 시 release의 현재 RDS/ElastiCache 데이터가 삭제됨(사용자 명시 동의), 다음 아침 실제 plan/apply로 최종 검증 필요**
