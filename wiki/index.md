...
- 🔴 (2026-08-18/19) backend actuator 헬스체크 스레드풀 공유 문제([qKet/backend#29](https://github.com/qKet/backend/issues/29)) — 2026-08-19 한때 이 세션이 `application.yml`(`management.server.port: 8081`)+`Infra`+`CD` 4개 레포에 코드 반영했으나, **팀원이 이미 별도로 해결 중임을 확인하고 중복 작업 방지를 위해 전부 원복**(커밋 전이라 `git restore`로 안전하게 되돌림) — **팀원 작업으로 진행 중, 이 세션은 관여 안 함**
- 🔴 (신규, 2026-08-18, 최우선) frontend 헬스체크 경로가 무거운 SSR 페이지(`/`)를 그대로 씀([qKet/frontend#28](https://github.com/qKet/frontend/issues/28)) — ALB 타겟그룹 헬스체크 연쇄 장애의 나머지 절반(backend#29는 위 항목 참고). [[troubleshooting/loadtest-10000-open-run-cascading-failures]] "문제 4" 참고 — **팀원이 별도 진행 중**
