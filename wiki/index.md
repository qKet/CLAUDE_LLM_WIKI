...
- 🔴 (신규, 2026-08-21) [[decisions/2026-08-21-04data-split-release-prod-directories]]에서 발견한 잠재 버그 — `modules/addons/eso`의 ESO 컨트롤러(`helm_release "external_secrets"`)가 release/prod 구분 없이 고정된 name/namespace를 씀. release는 이미 이 리소스를 apply했는데(살아있음), `04_data/prod`를 처음 apply하면 이름이 겹쳐서 Helm 설치가 실패할 가능성이 높음 — **아직 실제로 겪은 적은 없음(prod가 한 번도 apply된 적이 없어서), prod를 처음 켤 때 반드시 먼저 확인/조치 필요.** 후보 해결책: ESO 컨트롤러만 진짜 공유 singleton으로 `02_k8s-addon`으로 옮기기
